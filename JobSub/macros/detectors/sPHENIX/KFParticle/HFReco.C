...
//  string decayDescriptor = "K_S0 -> pi^+ pi^-";  //See twiki on how to set this
...
// string decayDescriptor = "[D0 -> K^- pi^+]cc";  //See twiki on how to set this
 string decayDescriptor = "[B+ -> K^+ pi^+ pi^-]cc";
 
 string reconstructionName = "MyBPTest";         //Used for naming output folder, file and nodes
...
void myHeavyFlavorReco()
{
  ...
  std::cout << "Use Zhaozhong's Codes" << std::endl;
  ...
  //Vertex parameters
  kfparticle->setMaximumVertexchi2nDOF(10);
  kfparticle->setMaximumDaughterDCA(0.1);

  //Parent parameters
  kfparticle->setMotherPT(0);
  kfparticle->setMinimumMass(5);
  kfparticle->setMaximumMass(6);


  //Intermediate parameters
  std::vector<std::pair<float, float>> intermediate_mass_range;
  intermediate_mass_range.push_back(make_pair(1.5, 2.1));
  ...
  std::vector<float> intermediate_min_dira = {0.0};
