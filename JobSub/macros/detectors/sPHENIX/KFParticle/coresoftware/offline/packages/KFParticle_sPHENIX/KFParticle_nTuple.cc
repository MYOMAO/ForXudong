...
KFParticle_nTuple::KFParticle_nTuple()
  ...
  , m_tree(nullptr)
{
	std::cout << "KF Ntuple Updated with my own way" << std::endl;
}  // Constructor
...
void KFParticle_nTuple::initializeBranches()
{
  ...
  m_tree->Branch(TString(mother_name) + "_Dir_mass", &m_direct_mother_mass, TString(mother_name) + "_Dir_mass/F");
                                   ...
                                   int nPVs, int multiplicity)
{
  ...
  for (int i = 0; i < m_num_tracks_nTuple; ++i)
  {
	...
	MomPx = m_calculated_daughter_px[i] + MomPx;
	MomPy = m_calculated_daughter_py[i] + MomPy;
	MomPz = m_calculated_daughter_pz[i] + MomPz;
	MomE = m_calculated_daughter_pe[i] + MomE;
  ...
  m_direct_mother_mass = sqrt(MomE * MomE - MomPx * MomPx - MomPy * MomPy - MomPz * MomPz);
