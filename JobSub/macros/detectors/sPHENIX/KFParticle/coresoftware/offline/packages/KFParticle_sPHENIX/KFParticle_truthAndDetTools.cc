...
float TruthParentProdX[20];
float TruthParentProdY[20];
float TruthParentProdZ[20];


float TruthParentDecayX[20];
float TruthParentDecayY[20];
float TruthParentDecayZ[20];



float TruthParentPx[20];
float TruthParentPy[20];
float TruthParentPz[20];


int GrandParentPDGID[20];
int GrandParentTrkID[20];
...
void KFParticle_truthAndDetTools::initializeTruthBranches(TTree *m_tree, int daughter_id, std::string daughter_number, bool m_constrain_to_vertex_truthMatch)
{
	...
	m_tree->Branch(TString(daughter_number) + "_true_track_history_pT", &m_true_daughter_track_history_pT[daughter_id]);
	m_tree->Branch(TString(daughter_number) + "_true_track_history_pT", &m_true_daughter_track_history_pT[daughter_id]);
//	m_tree->Branch("TrackPrePx", &TrackPrePx[daughter_id],"TrackLastPx/F");

	m_tree->Branch(TString(daughter_number) + "_TrackPrePx", &TrackPrePx[daughter_id], TString(daughter_number) + "_TrackPrePx/F");
	m_tree->Branch(TString(daughter_number) + "_TrackPrePy", &TrackPrePy[daughter_id], TString(daughter_number) + "_TrackPrePy/F");
	m_tree->Branch(TString(daughter_number) + "_TrackPrePz", &TrackPrePz[daughter_id], TString(daughter_number) + "_TrackPrePz/F");

	m_tree->Branch(TString(daughter_number) + "_TrackPreX", &TrackPreX[daughter_id], TString(daughter_number) + "_TrackPreX/F");
	m_tree->Branch(TString(daughter_number) + "_TrackPreY", &TrackPreY[daughter_id], TString(daughter_number) + "_TrackPreY/F");
	m_tree->Branch(TString(daughter_number) + "_TrackPreZ", &TrackPreZ[daughter_id], TString(daughter_number) + "_TrackPreZ/F");

/*	
	   m_tree->Branch(TString(daughter_number) + "_true_track_nmaps", &gnmaps[daughter_id], TString(daughter_number) + "_true_nmaps/I");
	   m_tree->Branch(TString(daughter_number) + "_true_track_nintt", &gnintt[daughter_id], TString(daughter_number) + "_true_nintt/I");
	   m_tree->Branch(TString(daughter_number) + "_true_track_ntpc", &gntpc[daughter_id], TString(daughter_number) + "_true_ntpc/I");
*/	
	m_tree->Branch(TString(daughter_number) + "_reco_track_nmaps", &reconmaps[daughter_id], TString(daughter_number) + "_reco_nmaps/I");
	m_tree->Branch(TString(daughter_number) + "_reco_track_nintt", &reconintt[daughter_id], TString(daughter_number) + "_reco_nintt/I");
	m_tree->Branch(TString(daughter_number) + "_reco_track_ntpc", &recontpc[daughter_id], TString(daughter_number) + "_reco_ntpc/I");


	//Additional Truth Information

	m_tree->Branch(TString(daughter_number) + "_ParentPDGID", &ParentPDGID[daughter_id], TString(daughter_number) + "_ParentPDGID/I");
	m_tree->Branch(TString(daughter_number) + "_ParentTrkID", &ParentTrkID[daughter_id], TString(daughter_number) + "_ParentTrkID/I");


	m_tree->Branch(TString(daughter_number) + "_TruthParentProdX", &TruthParentProdX[daughter_id], TString(daughter_number) + "_TruthParentProdX/F");
	m_tree->Branch(TString(daughter_number) + "_TruthParentProdY", &TruthParentProdY[daughter_id], TString(daughter_number) + "_TruthParentProdY/F");
	m_tree->Branch(TString(daughter_number) + "_TruthParentProdZ", &TruthParentProdZ[daughter_id], TString(daughter_number) + "_TruthParentProdZ/F");


	m_tree->Branch(TString(daughter_number) + "_TruthParentDecayX", &TruthParentDecayX[daughter_id], TString(daughter_number) + "_TruthParentDecayX/F");
	m_tree->Branch(TString(daughter_number) + "_TruthParentDecayY", &TruthParentDecayY[daughter_id], TString(daughter_number) + "_TruthParentDecayY/F");
	m_tree->Branch(TString(daughter_number) + "_TruthParentDecayZ", &TruthParentDecayZ[daughter_id], TString(daughter_number) + "_TruthParentDecayZ/F");



	m_tree->Branch(TString(daughter_number) + "_TruthParentPx", &TruthParentPx[daughter_id], TString(daughter_number) + "_TruthParentPx/F");
	m_tree->Branch(TString(daughter_number) + "_TruthParentPy", &TruthParentPy[daughter_id], TString(daughter_number) + "_TruthParentPy/F");
	m_tree->Branch(TString(daughter_number) + "_TruthParentPz", &TruthParentPz[daughter_id], TString(daughter_number) + "_TruthParentPz/F");



	m_tree->Branch(TString(daughter_number) + "_GrandParentPDGID", &GrandParentPDGID[daughter_id], TString(daughter_number) + "_GrandParentPDGID/I");
...
void KFParticle_truthAndDetTools::fillTruthBranch(PHCompositeNode *topNode, TTree * /*m_tree*/, KFParticle daughter, int daughter_id, KFParticle vertex, bool m_constrain_to_vertex_truthMatch)
{
	...
	//std::cout << "Here ok - KFParticle_truthAndDetTools" << std::endl;
...
//	std::cout << "track: "  << "  px = " << track->get_px() <<  "  x = " << track->get_x() << std::endl;

	float TrackLastPx = track->get_px();

	float TrackLastPy = track->get_py();
	float TrackLastPz = track->get_pz();

	float TrackLastX = track->get_x();
	float TrackLastY = track->get_y();
	float TrackLastZ = track->get_z();
	...
	//Add Mother and GrandMother Info
    ...
    {
      ParentPDGID[daughter_id] = 0;
    }
    else
    {
      mother = m_truthinfo->GetParticle(g4particle->get_parent_id());
      ParentPDGID[daughter_id] = mother->get_pid();
      ParentTrkID[daughter_id] = mother->get_track_id();
      TruthParentPx[daughter_id] = mother->get_px();
      TruthParentPy[daughter_id] = mother->get_py();
      TruthParentPz[daughter_id] = mother->get_pz();


	  PHG4VtxPoint *vtx = m_truthinfo->GetVtx(g4particle->get_vtx_id());
	  PHG4VtxPoint *ParentVtx = nullptr;
	  if (mother) ParentVtx = m_truthinfo->GetVtx(mother->get_vtx_id());
	  TruthParentProdX[daughter_id] = ParentVtx->get_x();
	  TruthParentProdY[daughter_id] = ParentVtx->get_y();
	  TruthParentProdZ[daughter_id] = ParentVtx->get_z();
	
	
	  TruthParentDecayX[daughter_id] = vtx->get_x();
	  TruthParentDecayY[daughter_id] = vtx->get_y();
	  TruthParentDecayZ[daughter_id] = vtx->get_z();
	
      if (mother->get_parent_id() == 0){
		  GrandParentPDGID[daughter_id] = 0;
	  }else{

		Grandmother = m_truthinfo->GetParticle(mother->get_parent_id());
        GrandParentPDGID[daughter_id] = Grandmother->get_pid();
	  }
    }
	...
	if (m_constrain_to_vertex_truthMatch)
	{
		...
		reconmaps[daughter_id] = nmaps;
		reconintt[daughter_id] = nintt;
		recontpc[daughter_id] = ntpc;

		//std::cout << "m_true_daughter_pv_x[daughter_id] = " << m_true_daughter_pv_x[daughter_id] << std::endl;

/*
		m_true_daughter_ip[daughter_id] = TrackLastPx;
		m_true_daughter_ip_xy[daughter_id] = TrackLastPy;
		m_true_daughter_p[daughter_id] = TrackLastPz;
*/		  
        TrackPrePx[daughter_id] = TrackLastPx;
        TrackPrePy[daughter_id] = TrackLastPy;
        TrackPrePz[daughter_id] = TrackLastPz;
	     
		TrackPreX[daughter_id] = TrackLastX;
        TrackPreY[daughter_id] = TrackLastY;
        TrackPreZ[daughter_id] = TrackLastZ;
