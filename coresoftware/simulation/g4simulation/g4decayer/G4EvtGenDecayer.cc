  ...
  , fVerboseLevel(0)
{
  ...
  extraModels.clear();
  SetDecayTable("BPD0Pi.DEC",false);
}
...
//  std::cout << "EvtGen Processing PDG ID = " << pdgEncoding << std::endl;

  std::cout << "Input Particle PDGID = " << part->pdg_id() << std::endl;
  stack.push(part);
  ...
  while (!stack.empty())
  {
    ...
    for (const auto& children : particle->children())
    {
	  ...
	  std::cout << "Children PDG ID: " << pdg << std::endl;
...
{
	//decayTable = "BPD0Pi.DEC";
	std::cout << "For This Analysis, my own forced decay table: " << decayTable << std::endl;
